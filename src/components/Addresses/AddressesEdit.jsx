...
import {addressDetail, addressUpdate} from "../../lib/api/AddresessApi.jsx";
...
export default function AddressesEdit (){
   ...
   async function handleUpdateAddress(e){
        ...
        if(response.status === 200){
           await alertSuccess("update address Success")
       }else{
    ...
    async function fetchAddress(token, id, addressId){
    ...
    useEffectOnce(() => {
        fetchContact()
            .then(()=> console.log("success fetch contact"))
        fetchAddress(token, id, addressId)
            .then(()=> console.log("success fetch address"))
    })
