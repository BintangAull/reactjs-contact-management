...
import {addressList, removeAddress} from "../../lib/api/AddresessApi.jsx";
...
export default function ContactDetail() {
    ...
    async function handleDelete(addressId){
       if (!await alertConfirm("serius ni mau di hapus ?")){
           return;

       }else {
           const response = await removeAddress(token, id, addressId)
           const responseBody = response.json();
           console.log(responseBody)
           if (response.status === 200){
               await alertSuccess("berhasil hapus alamat")
           }else {
