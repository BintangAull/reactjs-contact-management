...
import {contactList, removeContact} from "../../lib/api/ContactApi.jsx";
...
export default function ContactList() {
    ...
    function getPages(){
     ...
     async function handleDeleteContact(id) {
        if(!await alertConfirm("Are you sure to delete this contact?")){
            return;
        }

            const response = await removeContact(token, id)
            const responseBody = await response.json();
            if(response.status === 200){
                await alertSuccess("Contact has been deleted")
                setReload(!reload)
            }else  {
    ...
    return <>
            ...
            <div className="mt-10 flex justify-center">
                <nav className="flex items-center space-x-3 bg-gray-800 bg-opacity-80 rounded-xl shadow-custom border border-gray-700 p-3 animate-fade-in">
                    {page > 1 && <a href="#" onClick = {()=> handlePageChange(page-1)}
                    ...
                    {getPages().map(value => {
                        if(value === page){
                            return <a href="#" onClick={()=> handlePageChange(value)}
                    ...
                    {page < totalPage &&  <a href="#" onClick={() => handlePageChange(page+1)}
                                             className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 flex items-center"
                    >
